...
\begin{equation}
  ...
  \mathrm{q}^*
\end{equation}
where $\mathbf{v}^N$ and $\mathbf{v}^B$ are the same vector described in a inertial frame $N$ and the robot frame $B$ respectively.
A rotation matrix is another description of rotations
\begin{equation}
  \mathbf{v}^N = \mathrm{R}\left(\mathbf{v}^B\right)
\end{equation}
Importantly for the model node, rotation matrices collect the dot product of basis vectors: $\mathrm{R}_{ij} = \mathbf{\hat{n}}_i \cdot \mathbf{\hat{b}}_j$. In terms of quaternion $(x, y, z, w)$
\begin{equation}
  \mathrm{R}=\left(
  \begin{smallmatrix}
    x^2-y^2-z^2+w^2 & 2xy+2zw & 2xz-2yw \\
    2xy-2zw & -x^2+y^2-z^2+w^2 & -2xw+2yz \\
    2xz+2yw & 2xw+2yz & -x^2-y^2+z^2+w^2
  \end{smallmatrix} 
  \right) \label{EQN:RotationMatrix}
\end{equation}
That rate of change of the quaternion can be calculated from the angular velocity
\begin{equation}
  \mathrm{\dot{q}} = \frac{1}{2} \mathrm{q} \otimes
  \begin{pmatrix}
    \boldsymbol\omega^N_B \\
    0
  \end{pmatrix} \label{EQN:QuaternionVelocity}
\end{equation}
where $\boldsymbol\omega^N_B$ is the angular velocity of the robot $B$ relative to the inertial frame $N$, expressed in the body basis.
\begin{equation}
...
We begin by describing the motion of the quadcopter center of mass.
First, the motion of the quadcopter center of mass $c$ from its origin point $o$ is expressed simply
\begin{equation}
  \mathbf{p}_{c/o} = p_x \mathbf{\hat{n}}_x + p_y \mathbf{\hat{n}}_y + p_z \mathbf{\hat{n}}_z
\end{equation}
One way to find the velocity is the derivative in frame $N$
\begin{equation}
  \mathbf{v}^N_c = \dot{p}_x \mathbf{\hat{n}}_x + \dot{p}_y \mathbf{\hat{n}}_y + \dot{p}_z \mathbf{\hat{n}}_z \label{EQN:VelocityInertial}
\end{equation}
Expressing the velocity of the center of mass in the body frame simplifies the dynamcis and keeps the position representation useful.
The inertial velocity is now expressed in the body basis
\begin{equation}
  \mathbf{v}^N_c = v_x \mathbf{\hat{b}}_x + v_y \mathbf{\hat{b}}_y + v_z \mathbf{\hat{b}}_z \label{EQN:VelocityBody}
\end{equation}
Applying the vector transport theorem
\begin{equation}
  \mathbf{a}^N_c &= \mathbf{a}^B_c + {\boldsymbol\omega^N_B}\times\mathbf{v}^N_c
\end{equation}
The first term is the body center acceleration
\begin{equation}
  \mathbf{a}^B_c = \dot{v}_x \mathbf{\hat{b}}_x + \dot{v}_y \mathbf{\hat{b}}_y + \dot{v}_z \mathbf{\hat{b}}_z
\end{equation}
The second is the Coriolis term
\begin{align}
  {\boldsymbol\omega^N_B}\times\mathbf{v}^N_c &= \left(\omega_yv_z-\omega_zv_y\right) \mathbf{\hat{b}}_x \nonumber\\&+ \left(\omega_zv_x-\omega_xv_z\right) \mathbf{\hat{b}}_y \nonumber\\&+ \left(\omega_xv_y-\omega_yv_x\right) \mathbf{\hat{b}}_z
\end{align}
...
\subsection{State Trajectory}
The first differential equations come from combining equations \ref{EQN:VelocityInertial} and \ref{EQN:VelocityBody}, which is simply the vector of ($v_x$, $v_y$, $v_z$) rotated by $q$
\begin{align}
  \dot{p}_x &= \\
  \dot{p}_y &= v_y \\
